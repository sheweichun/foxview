 ...
 */
export function getStyle(node:HTMLElement, name:string) {
    /* istanbul ignore if */
    if (!hasDOM || !node) {
        return null;
    }
 ...
 * });
 */
export function setStyle(node, name:INodeMapType<string> | string, value?:string) {
    /* istanbul ignore if */
    if (!hasDOM || !node) {
        return false;
    }
    ...
    if (typeof name === 'object' && arguments.length === 2) {
        ...
        name = floatMap[name as string]
            ? 'cssFloat' in node.style
                ? 'cssFloat'
                : 'styleFloat'
            : name;
        if (typeof value === 'number' && PIXEL_PATTERN.test(name as string)) {
            value = `${value}px`;
        }
