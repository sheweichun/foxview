...
export default class Position {
    ...
    pinElement:Element | string
    baseElement:Element | string
    align:string
    offset:number[]
    needAdjust:boolean
    isRtl:boolean
    ...
    setPosition() {
        ...
        if (dom.getStyle(pinElement as HTMLElement, 'position') !== 'fixed') {
        ...
        if (
            ...
            dom.getStyle(baseElement as HTMLElement, 'position') !== 'fixed'
        ) {
            isBaseFixed = false;
        } else {
            isBaseFixed = true;
    ...
    _getParentOffset(element) {
        ...
        offset.top += parseFloat(dom.getStyle(parent, 'border-top-width'));
    ...
    _normalizeElement(element, isPinFixed):INormalResult {
        const result:INormalResult = {
                element: element,
                x: 0,
                y: 0,
            },
