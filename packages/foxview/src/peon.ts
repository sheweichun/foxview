...
class AttributePeon implements Peon {
  ...
  commit() {
    if (!this._shouldUpdate || this._isRef) return;
    this.node.setAttribute(this.name, this._pendingValue);
...
class EventPeon implements Peon {
  ...
  handleEvent(event: Event) {
    ...
    if (typeof this.value === 'function') {
      this.value.call(this.eventContext || this.node, event);
    } else {
