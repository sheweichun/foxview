...
import {Peon,ITemplateResult,ProcessResult, RenderOptions,IComponentConstructor,ComponentPropSchema,ComponentSlotSchema} from './type';
...
function _prepareSlot(node:Element,partIndex:number,peons: Array<Peon>,extra:HandlerExtra){
...
function forEach_node_attribute(node:Element,
    partIndex:number,
    templateResult:ITemplateResult,
    hasMarker:(name:string,target:AttributeMapItem,partIndex:number,strings:string[])=>void,
...
function _prepareComponent(ComponentProto:IComponentConstructor,node:Element,partIndex:number,peons: Array<Peon>,extra:HandlerExtra){
...
export function walkNode(fragment:DocumentFragment,templateResult:ITemplateResult,partIndex:number,option:RenderOptions):ProcessResult{
    const peons = [];
